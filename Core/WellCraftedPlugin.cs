...
namespace WellCrafted
{
    public sealed class WellCraftedPlugin : BaseSettingsPlugin<WellCraftedSettings>
    {
        ...
        // --- 100ms smoothing: remember last non-empty per choice while panel is visible ---
        private readonly string[] _lastNonEmpty = new string[3];
        private readonly long[] _lastNonEmptyAtMs = new long[3];
        private readonly System.Diagnostics.Stopwatch _clock = System.Diagnostics.Stopwatch.StartNew();
        private bool _wasRootVisible = false;
        ...
        {
            if (Settings == null || !Settings.Enable.Value) return;
                ...
                if (!root.IsVisible)
{
    ...
    // panel closed → reset smoothing state
    _wasRootVisible = false;
    Array.Clear(_lastNonEmpty, 0, _lastNonEmpty.Length);
    Array.Clear(_lastNonEmptyAtMs, 0, _lastNonEmptyAtMs.Length);
    return s;
}

s.Root = root;
s.RootOk = true;
_wasRootVisible = true;
                ...
                // Extract text (draw instantly, but smooth 100ms empties)
for (int i = 0; i < 3; i++)
{
    var element = s.Choices[i];
    var text = Guard.GetElementText(element);

    // If we momentarily got empty, reuse last non-empty for ≤100ms
    if (string.IsNullOrWhiteSpace(text))
    {
        var now = _clock.ElapsedMilliseconds;
        var lastMs = _lastNonEmptyAtMs[i];
        if (!string.IsNullOrWhiteSpace(_lastNonEmpty[i]) && (now - lastMs) <= 100)
        {
            text = _lastNonEmpty[i]; // reuse once, no logging, no delay
        }
    }
    else
    {
        // Remember this non-empty read for potential reuse next frame
        _lastNonEmpty[i] = text;
        _lastNonEmptyAtMs[i] = _clock.ElapsedMilliseconds;
    }
